...
- file: oci_ocssw_l2gen.md
  title: md
- file: ../../_downloads/notebooks/oci/oci_ocssw_l2gen.ipynb
  title: ipynb
jupytext:
  text_representation:
    extension: .md
...
Let's use the `earthaccess` Python package to access a L1B and a corresponding L2 file. Indexing -1 selects the last file in `results`.
...
Note that the L2 file we downloaded contains the entire granule, whereas our homegrown L2 file was generated only for the region bounded by 35 to 39 N and -76 to -74.5 W. To make an apples-to-apples comparison, we first need to subset the full L2 granule to the same geographic extent. OCSSW provides tools for this task through a simple two-step workflow.

Because L2 files store latitude and longitude as 2D arrays, we cannot directly subset the data using `xarray`'s standard `slice()` method. Instead, we must identify the corresponding scan line and pixel ranges that encompass our region of interest. The OCSSW utility `lonlat2pixline` converts geographic coordinates into the row and column indices needed for subsetting. As with other OCSSW programs, you can view all available options by running the command without any arguments.

To use `lonlat2pixline`, we first need the full path to the input Level-2 file.
