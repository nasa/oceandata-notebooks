...
Since we merged the MAPOL_OCEAN and _LAND suites, we now also have continous aerosol data no matter the ground cover type:

Aerosol optical properties are provided for both fine and coarse mode aerosols (and are defined in the same format as SPEXone FastMAPOL L2 products for interoperability between sensors)
...
The suite also includes aerosol microphysical properties:
...
Feel free to explore the dataset and get more familiar with the available information.
...
To support Level-3 data processing, a quality flag is also defined, which is usually based on $\chi^2$ and $N$. For the HARP2 test data, we chose 
...
We can plot the $\chi^2$ and total number of reflectance and degree of linear polarization (DoLP) measurements to assess the quality of the retrievals we looked at in Section 5.
...
Evaluating the quality flags based on the $\chi^2$ and $N$, we can see only a small portion of data near center of swath reaches best quality (defined as quality_flag=0). With the future improvement of data calibration, more data with better quality will be vailable.
