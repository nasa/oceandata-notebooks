...
oci_bbp = oci_dt["geophysical_data"]["bbp_442"]
for item in ("longitude", "latitude"):
    oci_bbp[item] = oci_dt["navigation_data"][item]
oci_bbp
```
...
```{code-cell} ipython3
subset = (
    (oci_bbp["longitude"] >= -120.5)
    & (oci_bbp["longitude"] <= -119.0)
    & (oci_bbp["latitude"] >= 33.8)
    & (oci_bbp["latitude"] <= 34.5)
)
oci_bbp = oci_bbp.where(subset & (oci_bbp > 0))
oci_bbp
```
...
ax.set_extent([-120.5, -119.0, 33.8, 34.5], crs=crs)
...
twinotter_alt = twinotter_ds["GPS_Altitude_BUCHOLTZ"].values / 10
...
```{code-cell} ipython3
hslr_dt = xr.open_datatree(hsrl_paths[1], phony_dims="sort")
hslr_dt
```
...
## 6. Access glider backscattering data
...
fig.update_layout(
    title=dict(
        text="Multi-Platform 3D Ocean/Atmosphere Visualization (Santa Barbara)",
        y=0.995,
        yanchor="top",
        x=0.4,
        xanchor="center",
    ...
    scene=dict(
        ...
        camera=dict(eye=dict(x=-1.3, y=-1.3, z=0.2)),
    ),
    width=1100,
    height=800,
