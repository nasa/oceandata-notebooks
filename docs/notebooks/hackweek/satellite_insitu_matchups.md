...
Note: The `get_f0` function requires an OCI sensor data file from the OCSSW shared files. Make sure your path is set up to be able to read the OCSSWROOT environment variable! If not, you can always download the F0 (TSIS-1 0.1nm) file from https://oceandata.sci.gsfc.nasa.gov/rsr_tables/
</div>
