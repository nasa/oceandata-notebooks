...
# +
rhos_555 = dataset["rhos"].sel({"wavelength_3d": 555})
...
# The masked dataset `cloudmasked_dataset` will retain the structure and information contained in the original merged dataset, but will take pixels where the 9th bit is flipped and assign them as NaNs. We can plot the data again to see what has changed:

# +
cloud_masked_rhos_555 = cloudmasked_dataset["rhos"].sel({"wavelength_3d": 555})

fig = plt.figure(figsize=(8,5))
ax = plt.axes(projection=ccrs.PlateCarree())
ax.coastlines(linewidth=0.5)
ax.gridlines(draw_labels={"left": "y", "bottom": "x"},linewidth=0.25)
cloud_masked_rhos_555.plot(x='longitude', y='latitude', cmap='viridis', vmin=0)
...
# +
masked_dataset = dataset.where(~((dataset.l2_flags & (1 << 9)) != 0) & ((dataset.l2_flags & (1 << 1)) != 0))

masked_rhos_555 = masked_dataset["rhos"].sel({"wavelength_3d": 555})
...
land_rhos_555 = land_values["rhos"].sel({"wavelength_3d": 555})
...
# ## 5. Convert netCDF to GeoTIFF format
...
rhos = rhos.transpose('wavelength_3d', 'pixels_per_line', 'number_of_lines')
rhos

rhos.rio.set_spatial_dims(x_dim='number_of_lines', y_dim='pixels_per_line')
rhos.rio.crs

rhos.rio.write_crs("epsg:4326", inplace=True)

rhos.rio.to_raster('PACE_OCI.20240701T175112.L2.SFREFL.V2_0.NRT.tif')
